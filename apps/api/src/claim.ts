...
import { clientIp, corsHeaders, json, optionsResponse } from "./http";
...
export async function getSiteInfo(
  ...
  if (owner) {
    ...
    payload.deploys = await listDeploys(env, slug, 10);
  ...
  if (raw) {
    try {
      ...
      payload.deployId = meta.deployId ?? null;
    } catch {
      payload.deployId = null;
    }
  }
