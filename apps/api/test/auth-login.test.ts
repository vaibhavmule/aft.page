...
describe("auth login", () => {
  ...
  it("POST /v1/deploy with session rejects tenant origin", async () => {
    const email = "deploy-tenant@example.com";
    const user = await findOrCreateUser(env, email);
    const session = await createSession(env, user.id);

    const stolen = await call(
      new Request(`${API_ORIGIN}/v1/deploy?slug=csrf-owned`, {
        method: "POST",
        headers: {
    ...
    const hub = await call(
      new Request(`${API_ORIGIN}/v1/deploy?slug=csrf-owned`, {
        ...
        body: "<h1>hub-owned</h1>",
      }),
    );
    expect(hub.status).toBe(200);
    expect(await getSiteOwnerId(env, "csrf-owned")).toBe(user.id);
  });
