...
describe("ops.aft.page host", () => {
  ...
  it("shows health, failures, and log links for ops email", async () => {
    ...
    expect(html).toContain("domain-groups");
    expect(html).toContain("Customer domains");
    expect(html.indexOf(">Status<")).toBeLessThan(html.indexOf(">Business<"));
    expect(html.indexOf(">Business<")).toBeLessThan(html.indexOf(">Cost (MTD)<"));
    expect(html.indexOf("Deploys work")).toBeLessThan(html.indexOf("Time to URL"));
    ...
    expect(html).toContain("Deploys work");
