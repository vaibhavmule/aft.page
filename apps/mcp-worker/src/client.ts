...
export async function getRepoJob(
  ...
  const res = await api.fetch(`${api.apiBase}/v1/jobs/${encodeURIComponent(jobId)}`, {
...
export async function deployRepo(
  ...
  const res = await api.fetch(`${api.apiBase}/v1/repo/deploy`, {
    method: "POST",
    headers: {
      "content-type": "application/json",
      "x-aft-client": "mcp",
  ...
  if (res.status === 202 && typeof body.jobId === "string") {
    ...
    while (Date.now() < deadline) {
      snap = await getRepoJob(api, body.jobId);
