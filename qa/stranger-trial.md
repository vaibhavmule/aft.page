...
- **2026-09-02:** Pre-trial dry run passed. Express fixture live at `nodejs-getting-started-sand.aft.page` (job `run_d8ac0075a22e`, HTTP 200), claimed by `hello@aft.page` so it is not subject to anon expiry/GC. Fixture stays ops-only (cron-probed, never on public status). Stranger path dry-run: `curl -fsSL https://aft.page/install | sh` → `aft deploy tiny` → live `https://tiny.aft.page` (HTTP 200). `/run/` form + `run.js` → `/v1/repo/deploy` → `/v1/jobs/{id}` confirmed working. **0 / 5 trials recorded. Next: send one stranger to https://aft.page/run.**
- **2026-09-06:** Inbound is working without a trial: **17 external accounts, 59 claimed sites**, first 12 Aug, 14 of them 28 Aug – 5 Sep, zero outreach sent (`ops.aft.page/users?filter=external` · docs/EVIDENCE-PACK.md § Ops snapshot). **That does not fill this table — still 0 / 5 trials recorded.** These rows need identity, repo, human T2U, and observed return; an inbound signup gives none of that. Two ways forward, do both: (a) run the five observed trials as written; (b) email the 17 and convert answers into named rows. Do not backfill this table from D1 — a claimed site is not a trial.
