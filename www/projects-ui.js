...
(function (global) {
  ...
  function renderThumb(url, slug) {
    ...
    if (url) {
      ...
      frame.className = "project-card-thumb-frame";
      frame.src = url;
      frame.loading = "lazy";
      frame.tabIndex = -1;
      frame.title = `Preview of ${slug}`;
  ...
  function closeShareModal() {
    ...
    if (shareFocusReturn) { shareFocusReturn.focus(); shareFocusReturn = null; }
  }
  ...
  async function openShareModal(opts) {
    ...
    try {
      const res = await fetch(`${API}/v1/sites/${encodeURIComponent(slug)}`, { credentials: "include" });
    ...
    if (isOwner) {
      ...
      sel.innerHTML = '<option value="private">Just me</option><option value="public">Everyone</option>';
      sel.value = vis;
      sel.addEventListener("change", async () => {
        ...
        try {
          ...
          if (!res.ok) { sel.value = vis; return; }
          if (opts.onChange) opts.onChange();
        } catch (_) { sel.value = vis; }
    ...
    visit.className = "btn"; visit.href = live; visit.target = "_blank"; visit.rel = "noopener"; visit.textContent = "Visit";
    const copy = document.createElement("button");
    copy.type = "button"; copy.className = "btn ghost"; copy.textContent = "Copy link";
    ...
    closeBtn.type = "button"; closeBtn.className = "share-modal-close";
    ...
    const onKey = (e) => {
      if (e.key === "Escape") { closeShareModal(); document.removeEventListener("keydown", onKey); }
    };
    document.addEventListener("keydown", onKey);
    visit.focus();
  }
  global.AftProjectsUI = { API, formatRelativeTime, visibilityMeta, iconSvg, renderThumb, openShareModal, closeShareModal };
})(window);
